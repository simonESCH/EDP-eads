...
#define TEST_TH 1
#define TEST_VP 1//(ordre mesH=2 ET THch=1 marche pas, marche avec mesh=1)
...
int main(int argc, char* argv[])
{
    ...
    Feel::cout<<"define ORDRE : ["<<ORDRE_TEMPERATURE<<","<<ORDRE_MESH<<"]\n";
    ...
    auto Th = Pch< ORDRE_TEMPERATURE >(mesh_ttl);//P continue
    Feel::cout<<"\nvous etes ici \n";
    auto ut = Th->element("temperature");
    auto u = Th->element("temperature test");
    //auto app_sol = Th->element("temperature");
    toc("Th");
    ...
    auto Th_vect = Pchv< ORDRE_TEMPERATURE >(mesh_ttl);
    ...
    if (! soption("Modele.choix").compare("POISEUILLE"))
    {
        tic();
        auto e =exporter(_mesh=mesh);
        e->add("test", vt);
        e->save();
        toc("export poiseuille");
    }
#endif

    tic();
    // interpolation  permet de faire passer le courant de mesh_Air dans le maillage mesh_ttl 
    /// \bug je n'arrive pas a le faire fonctionner quand test avec Pchv, Pdhv
    //auto AirToBeta = opInterpolation( _domainSpace = vt.functionSpace(), _imageSpace = beta.functionSpace());//, _range = markedelements(mesh,"POI") );
    //auto AirToBeta = operatorinterpolation(vt.functionSpace(), beta.functionSpace, 
    Feel::cout<<"creation de l'operateur d'interpolation\n";
    ...
    l = integrate(_range = markedelements(mesh_ttl, "IC1"), _expr = rapport_qk * id(u));
    l+= integrate(_range = markedelements(mesh_ttl, "IC2"), _expr = rapport_qk * id(u));
    toc("l");


    tic();
    auto a = form2( _trial=Th, _test=Th, _backend = backend_ttl);
    a = integrate( _range = elements(mesh_ttl), _expr =  inner(gradt(ut), grad(u)));//diffusion de chaleur

    if(soption("Modele.choix").compare("NO_AREATION"))
    {
        //auto N = expr<FEELPP_DIM,1>("{0,1}");
        double rapport_rck = doption("Air.rc")/doption("Air.k");
        a+= integrate( _range = markedelements(mesh_ttl, "POI"), _expr = rapport_rck * (gradt(ut) * idv(beta)) * id(u));//convection de la temperature
        //a+= integrate( _range = markedelements(mesh_ttl, "POI"), _expr = rapport_rck * (gradt(ut) * idv(vt)) * id(u));//convection de la temperature
        a+= integrate( _range = markedfaces(mesh_ttl, "out"), _expr = rapport_rck * inner( idv(beta), N() ) * idt(ut) * id(u));//condition de sortie
        //a+= integrate( _range = markedfaces(mesh_ttl, "out"), _expr = rapport_rck * inner( idv(vt), N() ) * idt(ut) * id(u));//condition de sortie
    }//end if AREATION

    a+= on( _range = markedfaces(mesh_ttl, "in"), _rhs = l, _element = ut, _expr = cst(T_ambiante));//condition d'entre
    toc("a");
    ...
    tic();
    auto e =exporter(_mesh=mesh_ttl);
    ...
    tic();
    auto Tproc1 = mean(_range=markedelements(mesh_ttl,"IC1"),_expr=idv(ut));
    auto Tproc2 = mean(_range=markedelements(mesh_ttl,"IC2"),_expr=idv(ut));
    auto Tout = mean(_range=markedfaces(mesh_ttl,"out"),_expr=idv(ut));
    toc("mean proc");
    ...
    //  auto hMax = mesh_ttl->hMax();
    //auto diffL2 = normL2(_range=elements(mesh_ttl), _expr=idv(ut)-idv(app_sol));
    ...
    //{
    //    tic();//ATTENTION ON NE SAUVE QUE SI ON VERIFIE LE BON NOMBRE
