...
int main(int argc, char**argv )
{
    ...
    double hmax=mesh->hMax();
    double l2 = normL2(_range=elements(mesh), _expr=idv(u)-expr(solution) );
    double h1 = normH1(_range=elements(mesh), _expr=idv(u)-expr(solution),_grad_expr=gradv(u)-grad<2>(expr(soption("functions.g"))) );
    
    
    
    if(Environment::isMasterRank())// fait si c'est le processus 0
    {
        std::ofstream fich("/data/atlas_home/atlas_eschbach/feel/qs_laplacian/resultat/tableau.csv", ios::out | ios::app);
        if(fich)
        {
            fich<< setprecision (9)<<hmax<<","<<l2<<","<<h1<<","<<time_mesh<<","<<time_solve<<std::endl;
            //(double(fin-debut)/CLOCKS_PER_SEC)<<std::endl;
            std::cout<<"le fichier tableau.csv est"<<std::endl;
            fich.close();
        }
        else
